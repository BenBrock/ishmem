...
def main() -> int:
    ...
    try:
        ...
        npes = ishmem.n_pes()
        if npes < 2:
            raise RuntimeError("torch_queue_put_test.py requires at least 2 PEs")
        ...
        try:
            ...
            ishmem.put(dst, src, pe=next_pe, queue=stream)
            ishmem.quiet(queue=stream)

            ishmem.barrier_all()
            torch.xpu.synchronize()
            expect_equal("queued symmetric tensor put ring", dst.cpu().tolist(), [float(prev_pe + 10)] * 4)
        finally:
